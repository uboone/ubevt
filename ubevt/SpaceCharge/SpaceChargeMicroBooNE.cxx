...
{  
  ...
  if((fEnableCalSpatialSCE == true) || (fEnableCalEfieldSCE == true))
  {
    ...
    std::string fname;
    std::cout << "filename for the space charge effects file: " << fInputFilename << ", " << fname << std::endl;
    cet::search_path sp("FW_SEARCH_PATH");
  ...
  if((fEnableDataDrivenSpatialSCE == true) || (fEnableDataDrivenEfieldSCE == true))
  {
    ...
    //histograms are Dx, Dy, Dz, dEx/E0, dEy/E0, dEz/E0
    std::vector<std::string> Position = {"X","Y","Z"};
    std::vector<std::string> position = {"x","y","z"};

    //Get vector of THF3 histograms
    //Spatial distortion
    TH3F *histo;
    for(uint i=0; i<3; i++){
      TString histoName = Form("Distorted_v_%s", Position[i].c_str());
      gDirectory->GetObject(histoName, histo);
      TString newName = Form("hD%s", position[i].c_str());
      histo->SetName(newName);
      CalSCEhistograms.push_back(histo);
    }
    //E-field distortion
    for(uint i=0; i<3; i++){
      TString histoName = Form("Distorted_EField_%s", Position[i].c_str());
      gDirectory->GetObject(histoName, histo);
      TString newName = Form("hE%s", position[i].c_str());
      histo->SetName(newName);
      CalSCEhistograms.push_back(histo);
    }
...
bool spacecharge::SpaceChargeMicroBooNE::EnableDataDrivenSpatialSCE() const
{
...
bool spacecharge::SpaceChargeMicroBooNE::EnableDataDrivenEfieldSCE() const
{
