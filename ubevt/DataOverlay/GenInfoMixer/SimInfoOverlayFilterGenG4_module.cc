////////////////////////////////////////////////////////////////////////
// Class:       SimInfoOverlayFilterGenG4
// Plugin Type: filter (art v2_11_03)
// File:        SimInfoOverlayFilterGenG4_module.cc
//
// Generated at Sat Nov 24 16:09:21 2018 by Wesley Ketchum using cetskelgen
// Modified by C Thorpe to handle separate prior gen/g4/filtering passes
...
class mix::SimInfoOverlayFilterGenG4 : public art::EDFilter {
public:
  explicit SimInfoOverlayFilterGenG4(fhicl::ParameterSet const & p);
  // The compiler-generated destructor is fine for non-base
  // classes without bare pointers or other resource use.
  
  // Plugins should not be copied or assigned.
  SimInfoOverlayFilterGenG4(SimInfoOverlayFilterGenG4 const &) = delete;
  SimInfoOverlayFilterGenG4(SimInfoOverlayFilterGenG4 &&) = delete;
  SimInfoOverlayFilterGenG4 & operator = (SimInfoOverlayFilterGenG4 const &) = delete;
  SimInfoOverlayFilterGenG4 & operator = (SimInfoOverlayFilterGenG4 &&) = delete;
...
private:
  ...
  std::vector<art::InputTag>    fMCTruthMCParticleAssnsMCTruthLookupLabel; //Added C Thorpe
...
mix::SimInfoOverlayFilterGenG4::SimInfoOverlayFilterGenG4(fhicl::ParameterSet const & p)
  :
...
void mix::SimInfoOverlayFilterGenG4::FillInputModuleLabels(fhicl::ParameterSet const & p,
						      std::string s,
...
template<class T>
void mix::SimInfoOverlayFilterGenG4::DeclareProduces(std::vector<art::InputTag> const& labels)
{
  for(auto label : labels)
    this->produces<T>(label.instance());   
}

void mix::SimInfoOverlayFilterGenG4::DeclareProduces()
{
...
template<class T>
void mix::SimInfoOverlayFilterGenG4::InitializeCollectionMap(std::vector<art::InputTag> const& labels,
...
void mix::SimInfoOverlayFilterGenG4::InitializeCollectionMaps()
{
...
template<class T>
void mix::SimInfoOverlayFilterGenG4::PutCollectionOntoEvent(art::Event & e,
						       CollectionMap<T> & colmap)
{
  for(auto & cp : colmap)
...
void mix::SimInfoOverlayFilterGenG4::PutCollectionsOntoEvent(art::Event & e)
{
...
template<class T>
std::map< std::pair<art::ProductID,size_t> , art::Ptr<T> >
mix::SimInfoOverlayFilterGenG4::FillCollectionMap(std::vector<art::InputTag> const& labels,
					     ...
					     art::Event & e)
{
  ...
  for(auto label : labels){
    ...
    if(label.process().size()==0)
      throw cet::exception("SimInfoOverlayFilterGenG4")
...
template< class T, class U >
void mix::SimInfoOverlayFilterGenG4::FillAssnsCollectionMap(std::vector<art::InputTag> const& labels,
...
bool mix::SimInfoOverlayFilterGenG4::filter(art::Event & e)
{
  ...
  if(fMCTruthInputModuleLabels.empty() && !fMCTruthMCParticleAssnsInputModuleLabels.empty() && fMCTruthMCParticleAssnsMCTruthLookupLabel.size() == 1) {
    ...
    for(size_t mcl = 0; mcl < mclists.size(); ++mcl){
      ...
      if(process_name != moduleDescription().processName()) {
        ...
        process_name = fMCTruthMCParticleAssnsMCTruthLookupLabel.at(0).process();
        instance_name = fMCTruthMCParticleAssnsMCTruthLookupLabel.at(0).instance();
        module_name = fMCTruthMCParticleAssnsMCTruthLookupLabel.at(0).label();
...
void mix::SimInfoOverlayFilterGenG4::beginJob()
{
}

bool mix::SimInfoOverlayFilterGenG4::beginRun(art::Run & r)
{
  return true;
}

bool mix::SimInfoOverlayFilterGenG4::beginSubRun(art::SubRun & sr)
{
  if(fFillPOTInfo){
    fPOTSum_totpot = 0.0;
    fPOTSum_totgoodpot = 0.0;
...
void mix::SimInfoOverlayFilterGenG4::endJob()
{
}

bool mix::SimInfoOverlayFilterGenG4::endRun(art::Run & r)
{
  return true;
}

bool mix::SimInfoOverlayFilterGenG4::endSubRun(art::SubRun & sr)
{
  if(fFillPOTInfo){
...
void mix::SimInfoOverlayFilterGenG4::respondToCloseInputFile(art::FileBlock const & fb)
{
}

void mix::SimInfoOverlayFilterGenG4::respondToCloseOutputFiles(art::FileBlock const & fb)
{
}

void mix::SimInfoOverlayFilterGenG4::respondToOpenInputFile(art::FileBlock const  &fb)
{
}

void mix::SimInfoOverlayFilterGenG4::respondToOpenOutputFiles(art::FileBlock const & fb)
{
}

DEFINE_ART_MODULE(mix::SimInfoOverlayFilterGenG4)
